...
namespace ASG
{
    public partial class frm_facturas : Form
    {
        ...
        private void cargaDatos()
        {
            ...
            try
            {
                string sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text,comboBox5.SelectedIndex);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1),reader.GetString(2),reader.GetString(3), reader.GetString(4), reader.GetString(5), reader.GetString(6), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(7)),reader.GetString(8), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(10)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(11)), reader.GetString(13));
                    while (reader.Read())
                    {
                        dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2) ,reader.GetString(3), reader.GetString(4), reader.GetString(5), reader.GetString(6), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(7)), reader.GetString(8), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(10)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(11)), reader.GetString(13));
        ...
        private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if(dataGridView1.RowCount > 0)
            {
                var forma = new frm_detalleFactura(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells[9].Value.ToString(), dataGridView1.CurrentRow.Cells[10].Value.ToString(), dataGridView1.CurrentRow.Cells[7].Value.ToString(),dataGridView1.CurrentRow.Cells[2].Value.ToString(),dataGridView1.CurrentRow.Cells[1].Value.ToString(),true, dataGridView1.CurrentRow.Cells[8].Value.ToString(), dataGridView1.CurrentRow.Cells[4].Value.ToString(), dataGridView1.CurrentRow.Cells[3].Value.ToString(), dataGridView1.CurrentRow.Cells[11].Value.ToString(),0, true);
                forma.ShowDialog();
            }
        }
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND NOMBRE LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
                    ...
                    else
                    {
                        ...
                        sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND NOMBRE LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                        ...
                        else
                        {
                            conexion.Close();
                            conexion = ASG_DB.connectionResult();
                            sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND TIPO_FACTURA LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                            cmd = new OdbcCommand(sql, conexion);
                            reader = cmd.ExecuteReader();
                            if (reader.Read())
                            {
                            ...
                            else
                            {
                                conexion.Close();
                                conexion = ASG_DB.connectionResult();
                                sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND NOMBRE_USUARIO LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                                cmd = new OdbcCommand(sql, conexion);
                                reader = cmd.ExecuteReader();
                                if (reader.Read())
                                {
                                ...
                                else
                                {
                                    conexion.Close();
                                    conexion = ASG_DB.connectionResult();
                                    sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND TOTAL_FACTURA LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                                    cmd = new OdbcCommand(sql, conexion);
                                    reader = cmd.ExecuteReader();
                                    if (reader.Read())
                                    {
                                    ...
                                    } else
                                    {
                                        conexion.Close();
                                        conexion = ASG_DB.connectionResult();
                                        sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND ID_CLIENTE LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                                        cmd = new OdbcCommand(sql, conexion);
                                        reader = cmd.ExecuteReader();
                                        if (reader.Read())
                                        {
                                        ...
                                        {
                                            conexion = ASG_DB.connectionResult();
                                            sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND ID_FACTURA LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                                            cmd = new OdbcCommand(sql, conexion);
                                            reader = cmd.ExecuteReader();
                                            if (reader.Read())
                                            {
                                            ...
                                            {
                                                conexion = ASG_DB.connectionResult();
                                                sql = string.Format("SELECT * FROM VISTA_FACTURAS WHERE NOMBRE_SUCURSAL  = '{0}' AND ESTADO_TUPLA = {1} AND DESCUENTO_FACTURA LIKE '%{2}%' ORDER BY FECHA_EMISION_FACTURA DESC LIMIT 165;", comboBox1.Text, comboBox5.SelectedIndex, textBox1.Text.Trim());
                                                cmd = new OdbcCommand(sql, conexion);
                                                reader = cmd.ExecuteReader();
                                                if (reader.Read())
                                                {
        ...
        {
            // formualrio que busca el proucto
            var forma = new frm_buscaEdicion(nombreSucursal, rolUs,1);
            if(forma.ShowDialog() == DialogResult.OK)
            {
               string temp = forma.CurrentMercaderia.getMercaderia;
                dataGridView1.Rows.Clear();
                OdbcConnection conexion = ASG_DB.connectionResult();
                try
                {
                    string sql = string.Format("SELECT * FROM VISTA_FACTURAS_PRODUCTOS WHERE ID_MERCADERIA =  '{0}' AND NOMBRE_SUCURSAL = '{1}' ORDER BY FECHA_VENCIMIENTO_FACTURA DESC LIMIT 150;", temp, comboBox1.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
