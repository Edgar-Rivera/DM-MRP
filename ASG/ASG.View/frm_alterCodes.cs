    ...
    public partial class frm_alterCodes : Form
    {
        ...
        private void deleteCodigo(string codigoMer, string codigoAlterno)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("NO SE HA ELIMINADO EL CODIGO ALTERNO!", "GESTION SUBCODIGOS", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private bool existeCodigo(string codigomercaderia, string subcodigoasingado)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("SELECT ID_STOCK FROM SUBCODIGOS_MERCADERIA WHERE ID_STOCK = '{0}' AND SUBCODIGO = '{1}';", codigomercaderia, subcodigoasingado);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void activaCodigo(string codigoAlterno)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("UPDATE SUBCODIGOS_MERCADERIA SET ESTADO_TUPLA = TRUE WHERE ID_STOCK = '{0}' AND SUBCODIGO = '{1}';", codigoMercaderia, codigoAlterno);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (dataGridView1.RowCount > 0) {
                for (int i = 0; i < dataGridView1.RowCount - 1; i++)
                {
                    ...
                    {
                        if (existeCodigo(codigoMercaderia, dataGridView1.Rows[i].Cells[1].Value.ToString()))
                        { 
                            //MessageBox.Show("EL CODIGO YA EXISTE! SE ACTIVARA");
                            activaCodigo(dataGridView1.Rows[i].Cells[1].Value.ToString());
                        }
                        else
                        {
