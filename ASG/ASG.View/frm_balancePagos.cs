    ...
    public partial class frm_balancePagos : Form
    {
        ...
        private void cancelaCuenta(double abono, string numeroCuenta)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("IMPOSIBLE ABONAR PAGO DEL PROVEEDOR!", "NUEVA VENTA - CREDITO", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void updateAbono(double abonoCuenta, string numeroCuenta)
        {
            ...
            try
            {
                ...
                string sql = string.Format("CALL ACTUALIZA_CUENTA_PROVEEDOR ({0},'{1}')", abonoCuenta, numeroCuenta);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
        ...
        private void abonaPagoCliente(string numeroCuenta, double abono)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("IMPOSILE ABONAR PAGO DEL PROVEEDOR!", "NUEVA VENTA - CREDITO", MessageBoxButtons.OK, MessageBoxIcon.Error);
