...
namespace ASG
{
    public partial class frm_cuentasPagar : Form
    {
        ...
        private void cargaDatosCompletados()
        {
            ...
            {
                dataGridView1.Rows.Clear();
                string sql = string.Format("SELECT * FROM COMPRAS_CANCELADAS;");
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void buscaCompletas()
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT * FROM COMPRAS_CANCELADAS WHERE ID_PROVEEDOR LIKE '%{0}%';", textBox1.Text.Trim());
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    dataGridView1.Rows.Clear();
                    dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetString(4), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), reader.GetString(5), reader.GetString(6), reader.GetString(7));
                    while (reader.Read())
                    {
                        dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetString(4), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), reader.GetString(5), reader.GetString(6), reader.GetString(7));
                    }
                ...
                else
                {
                    ...
                    sql = string.Format("SELECT * FROM COMPRAS_CANCELADAS WHERE NOMBRE_PROVEEDOR LIKE '%{0}%';", textBox1.Text.Trim());
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if(dataGridView1.RowCount > 0)
            {
                if(e.KeyData == Keys.Enter)
                {
                    ...
                    var tester = new frm_balancePagos(dataGridView1.CurrentRow.Cells[1].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString(), rolUs, cast,nombreSucursal,usuario, dataGridView1.CurrentRow.Cells[3].Value.ToString());
                    tester.Show();
                }
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT * FROM VISTA_PAGAR WHERE ESTADO_TUPLA = {0} AND FECHA_PAGO LIKE '%{1}%';", 1, dateTimePicker1.Value.ToString("yyyy-MM-dd"));
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        {
            var xpose = new frm_AbonosProveedoresRealizados();
            xpose.Show();
        }
    }
}
