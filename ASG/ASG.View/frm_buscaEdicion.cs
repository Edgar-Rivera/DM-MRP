...
namespace ASG
{
    public partial class frm_buscaEdicion : Form
    {
        ...
        int tipo;
        public frm_buscaEdicion(string sucursal, string rol, int  clasificacion)
        {
            InitializeComponent();
            cargaSucursales();
        ...
        private void cargaMercaderiaClasificacion()
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE NOMBRE_SUCURSAL = '{0}' AND ID_CLASIFICACION = {1};", comboBox2.Text, tipo);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        internal frm_comprasM.Mercaderia CurrentMercaderia
        {
            get
            {
                return new frm_comprasM.Mercaderia()
                {
        ...
        private void buscaMercaderiaTotal()
        {
            if (textBox1.Text != "")
            {
                try
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        conexion.Close();
                                        conexion = ASG_DB.connectionResult();
                                        sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE ID_MERCADERIA LIKE '{0}%' AND NOMBRE_SUCURSAL = '{1}';", textBox1.Text, comboBox2.Text);
                                        cmd = new OdbcCommand(sql, conexion);
                                        reader = cmd.ExecuteReader();
                                        if (reader.Read())
                                        {
        ...
        private void buscaMercaderiaClasificacion()
        {
            if (textBox1.Text != "")
            {
                ...
                {
                    OdbcConnection conexion = ASG_DB.connectionResult();
                    string sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE ID_MERCADERIA LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
                    ...
                    else
                    {
                        conexion.Close();
                        conexion = ASG_DB.connectionResult();
                        sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE DESCRIPCION LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                        cmd = new OdbcCommand(sql, conexion);
                        reader = cmd.ExecuteReader();
                        if (reader.Read())
                        {
                        ...
                        else
                        {
                            conexion.Close();
                            conexion = ASG_DB.connectionResult();
                            sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE NOMBRE_PROVEEDOR LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                            cmd = new OdbcCommand(sql, conexion);
                            reader = cmd.ExecuteReader();
                            if (reader.Read())
                            {
                            ...
                            else
                            {
                                conexion.Close();
                                conexion = ASG_DB.connectionResult();
                                sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE PRECIO_COMPRA LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                                cmd = new OdbcCommand(sql, conexion);
                                reader = cmd.ExecuteReader();
                                if (reader.Read())
                                {
                                ...
                                else
                                {
                                    conexion.Close();
                                    conexion = ASG_DB.connectionResult();
                                    sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE TOTAL_UNIDADES LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                                    cmd = new OdbcCommand(sql, conexion);
                                    reader = cmd.ExecuteReader();
                                    if (reader.Read())
                                    {
                                    ...
                                    else
                                    {
                                        conexion.Close();
                                        conexion = ASG_DB.connectionResult();
                                        sql = string.Format("SELECT * FROM VISTA_MERCADERIA WHERE ID_MERCADERIA LIKE '{0}%' AND NOMBRE_SUCURSAL = '{1}' AND ID_CLASIFICACION = {2};", textBox1.Text, comboBox2.Text, tipo);
                                        cmd = new OdbcCommand(sql, conexion);
                                        reader = cmd.ExecuteReader();
                                        if (reader.Read())
                                        {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (Convert.ToDouble(tipo) > 2)
            {
                buscaMercaderiaTotal();
            }
            else
            {
                buscaMercaderiaClasificacion();
            }
        }
