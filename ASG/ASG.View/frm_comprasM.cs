...
namespace ASG
{
    public partial class frm_comprasM : Form
    {
        string nameUs;
        string rolUs;
        string usuario;
        ...
        public frm_comprasM(string nameUser, string rolUser, string user, string sucursal, bool[] privilegio)
        {
            InitializeComponent();
            usuario = user;
            nameUs = nameUser;
        ...
        private void cargaCompras()
        {
            ...
            try
            {
                ...
                string sql = string.Format("SELECT * FROM VISTA_COMPRA ORDER BY FECHA_COMPRA DESC LIMIT 165;");
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {          
                    dataGridView2.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(4)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(6)), reader.GetString(7),reader.GetString(8));
                    while (reader.Read())
                    {
                        dataGridView2.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(4)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(6)), reader.GetString(7), reader.GetString(8));
        ...
        {
            if (button3.Text == "Cerrar Compra" | button10.Text == "Cerrar Compra")
            {
                DialogResult result;
        ...
        private bool codigoRepetido(string code)
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_MATERIA_PRIMA FROM MATERIA_PRIMA WHERE ID_MATERIA_PRIMA =  '{0}';", code);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
...
/*
        ...
        private void textBox3_Leave(object sender, EventArgs e)
        {
            if (textBox4.Text != "")
            {
                if (textBox3.Text != "")
                {
                    textBox15.Text = Convert.ToString(Convert.ToDouble(textBox4.Text) + Convert.ToDouble(textBox3.Text));
                }
                else
                {
            ...
            else if (Convert.ToInt32(e.KeyData) == Convert.ToInt32(Keys.Control) + Convert.ToInt32(Keys.H))
            {
                if (compraActual != "")
                {
                    if (tabControl1.SelectedTab == tabPage1)
                    {
                        ...
                        {
                            MessageBox.Show("No se ha ingresado algun producto!", "Historial de Precios", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                    else if (tabControl1.SelectedTab == tabPage2)
                    {
      ...
      /*  private void nuevaCompra(string code)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (button3.Text == "Nueva Compra")
            {
                ...
                if (forma.ShowDialog() == DialogResult.OK)
                {
                    ...
                    textBox14.Text = forma.currentCompra.getProveedor;                 
                    currentProveedor = textBox14.Text;
                    textBox2.Focus();
                }
                else
                {
        ...
        private void ingresaDetalle()
        {
            if (compraActual != "")
            {
                ...
                {
                    OdbcConnection conexion = ASG_DB.connectionResult();
                    string sql = sql = string.Format("CALL DETALLE_COMPRA ('{0}','{1}',{2},{3},{4},{5});", compraActual,textBox2.Text.Trim(),textBox3.Text,Convert.ToDouble(textBox7.Text),0,numero);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    if (cmd.ExecuteNonQuery() == 1)
                    {
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            getCalculosEspecias(textBox11.Text);
                        }
                    }
                    else
                    {
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            {     
                    ...
                    if (Convert.ToDouble(textBox4.Text) > 0)
                    {
                        ...
                        if (forma.DialogResult == DialogResult.OK)
                        {
                            textBox7.Text = forma.CurrentPrecio.getPrecio;
                           
                        }
                        else if (forma.DialogResult == DialogResult.Yes)
                        {
                            textBox7.Text = forma.CurrentPrecio.getSubtotal;
                        }
        ...
        {
            DialogResult result;
            result = MessageBox.Show("¿ESTA SEGURO QUE DESEA CANCELAR LA OPERACION?", "COMPRAS - MATERIA PRIMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                textBox1.Text = "";
                textBox3.Text = "";
        ...
        private void dataGridView2_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (dataGridView2.RowCount > 0)
            {
                var forma = new frm_detalleCompra(dataGridView2.CurrentRow.Cells[0].Value.ToString(), dataGridView2.CurrentRow.Cells[4].Value.ToString(), dataGridView2.CurrentRow.Cells[5].Value.ToString(), dataGridView2.CurrentRow.Cells[6].Value.ToString(), true);
                forma.ShowDialog();
            }
        }
        ...
        {
            var forma = new frm_buscaMateriaPrima(sucursalDestino, rolUs, false);
            if (forma.ShowDialog() == DialogResult.OK)
            {
        ...
        private int getSucursales(string mercaderiaC)
        {
            ...
            try
            {
                string sql = string.Format("SELECT COUNT(NOMBRE_SUCURSAL) FROM VISTA_SUCURSALES_PRECIOS_MATERIA WHERE ID_MATERIA_PRIMA = '{0}';", mercaderiaC);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void button33_Click(object sender, EventArgs e)
        {
           ...
           {
                if ( (textBox2.Text != "") && (textBox7.Text != "") && (textBox3.Text != ""))
                {
                    DialogResult result;
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        if (!existeMercaderia(textBox2.Text, sucursalDestino))
                        {
                            // NO existe la mercaderia
                            OdbcConnection conexion = ASG_DB.connectionResult();
                            try
                            {
                                string sql = string.Format("INSERT INTO MATERIA_PRIMA VALUES ('{0}'," +
                                    "(SELECT ID_PROVEEDOR FROM PROVEEDOR WHERE NOMBRE_PROVEEDOR = '{1}')," +
                                    "(SELECT ID_UNIDAD_MEDIDA FROM UNIDAD_MEDIDA WHERE NOMBRE = '{2}'),'{3}', TRUE);",
                                    textBox2.Text, textBox14.Text, comboBox3.Text, textBox1.Text);
                                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                                if (cmd.ExecuteNonQuery() == 1)
                                {
                                    if (agregaCantidadEnSucursal())
                                    {
                                        if (agregaProductosCompra())
                                        {
                                            var price = new frm_PrecioMateriaPrimaS(textBox2.Text, sucursalDestino);
                                            price.ShowDialog();
                                            nuevoProducto();
                        ...
                        {
                            // la mercaderia existe
                            OdbcConnection conexion = ASG_DB.connectionResult();
                            try
                            {
                                string sql = string.Format("UPDATE  MATERIA_PRIMA SET DESCRIPCION = '{0}', " +
                                    "ID_PROVEEDOR = (SELECT ID_PROVEEDOR FROM PROVEEDOR WHERE NOMBRE_PROVEEDOR = '{1}') WHERE ID_MATERIA_PRIMA = '{2}';",
                                    textBox1.Text, textBox14.Text, textBox2.Text);
                                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                                if ((cmd.ExecuteNonQuery() == 1) | (cmd.ExecuteNonQuery() == 0))
                                {
                                    if (ActualizaCantidadEnSucursal())
                                    {
                                        if (agregaProductosCompra())
                                        {
                                            var price = new frm_PrecioMateriaPrimaS(textBox2.Text, sucursalDestino);
        ...
        private void button25_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                var forma = new frm_HistorialPrecios(textBox2.Text.Trim(), sucursalDestino);
                forma.Show();
            } else
            {
        ...
        {
            if (textBox8.Text != "") {
                var forma = new frm_HistorialPrecios(textBox8.Text.Trim(), sucursalDestino);
                forma.Show();
            } else
            {
                MessageBox.Show("No ha seleccionado ningun producto!", "Historial de Precios", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        // DESDE AQUI EMPIEZA TODO LO QUE TIENE QUE VER CON LO NEUVO DE DM OCCIDENTE ******************************
        private bool existeMercaderia(string code, string sucursal)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("SELECT ID_MATERIA_PRIMA FROM SUCURSAL_HAS_MATERIA_PRIMA WHERE ID_MATERIA_PRIMA =  '{0}'" +
        ...
        private bool agregaCantidadEnSucursal()
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("INSERT INTO SUCURSAL_HAS_MATERIA_PRIMA VALUES " +
                    ...
                    sucursalDestino, textBox2.Text, textBox3.Text, textBox7.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    return true;
        ...
        private bool ActualizaCantidadEnSucursal()
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("UPDATE SUCURSAL_HAS_MATERIA_PRIMA " +
                    ...
                    textBox7.Text, textBox3.Text, textBox2.Text, sucursalDestino);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    return true;
        ...
        private bool agregaProductosCompra()
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("INSERT INTO CONTROL_COMPRA VALUES " +
                    "(NULL,{0},'{1}',{2},{3},0,{4}, TRUE);",
                    compraActual, textBox2.Text, textBox3.Text, textBox7.Text, 
                    Convert.ToDouble(textBox3.Text) * Convert.ToDouble(textBox7.Text));
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    return true;
        ...
        {
            if ((textBox1.Text != "") && (textBox2.Text != "") && (textBox7.Text != "") && (comboBox3.Text != "") & (textBox3.Text != ""))
            {
                DialogResult result;
                result = MessageBox.Show("¿DESEA GUARDAR LA MATERIA PRIMA EN LA COMPRA?" + "\n" + "  LA MATERIA PRIMA SE AGREGARA A LA COMPRA", "COMPRAS - MATERIA PRIMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == System.Windows.Forms.DialogResult.Yes)
                {
                    if (!existeMercaderia(textBox2.Text, sucursalDestino))
                    {
                        ...
                        try
                        {
                            string sql = string.Format("INSERT INTO MATERIA_PRIMA VALUES ('{0}'," +
                                "(SELECT ID_PROVEEDOR FROM PROVEEDOR WHERE NOMBRE_PROVEEDOR = '{1}')," +
                                "(SELECT ID_UNIDAD_MEDIDA FROM UNIDAD_MEDIDA WHERE NOMBRE = '{2}'),'{3}',{4}, TRUE);", 
                                textBox2.Text, textBox14.Text,comboBox3.Text, textBox1.Text, textBox7.Text);
                            OdbcCommand cmd = new OdbcCommand(sql, conexion);
                            if (cmd.ExecuteNonQuery() == 1)
                            {
                                if (agregaCantidadEnSucursal())
