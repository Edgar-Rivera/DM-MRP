...
namespace ASG
{
    public partial class frm_proveedores : Form
    {
        ...
        string idSucursal;
        bool index;
        public frm_proveedores(string nameUser, string rolUser, string user, int x, string sucursal, bool indexer)
        {
            InitializeComponent();
            usuario = user;
            nameUs = nameUser;
        ...
        private bool nombreProveedorEdicion(string nombre)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("SELECT NOMBRE_PROVEEDOR FROM PROVEEDOR WHERE NOMBRE_PROVEEDOR <> '{0}' AND ESTADO_TUPLA = TRUE;", nombre);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private bool nombreProveedor(string nombre)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                if (nombreProveedor(textBox6.Text))
                {
                    try
                    {
                        OdbcConnection conexion = ASG_DB.connectionResult();
                ...
                {
                    MessageBox.Show("EL PROVEEDOR YA EXISTE!", "GESTION PROVEEDORES", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
        ...
        internal frm_comprasM.Proveedor CurrentProveedor
        {
            get
            {
                return new frm_comprasM.Proveedor()
                {
        ...
        private void dataGridView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {            
                ...
                } else
                {
                    if (dataGridView1.RowCount > 0)
                    {
                        var forma = new frm_vistaProveedor(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString(), dataGridView1.CurrentRow.Cells[3].Value.ToString());
                        forma.Show();
                    }
                }
            }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("SE CAMBIARA EL ID DEL PROVEEDOR!", "GESTION MERCADERIA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                bool codex = true;
                while (codex)
                {
                    string temp = createCode(9);
        ...
        private void frm_proveedores_MouseDoubleClick(object sender, MouseEventArgs e)
        {
