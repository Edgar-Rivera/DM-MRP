    ...
    public partial class frm_devoluciones : Form
    {
       ...
       // bool flags = false;
        ...
        private void duplicaFactura(string codigo)
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    label12.Text = ""+ reader.GetDouble(9);
        ...
        private bool poseeCuenta()
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_FACTURA FROM CUENTA_POR_COBRAR_CLIENTE WHERE ID_FACTURA = '{0}' AND ESTADO_TUPLA = TRUE;", textBox5.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private double totalAbonadoCliente()
        {
            ...
            try
            {
                string sql = string.Format("SELECT TOTAL_PAGADO FROM CUENTA_POR_COBRAR_CLIENTE WHERE ID_FACTURA = '{0}';", textBox5.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void actualizaCreditosClientes()
        {
            if (totalAbonadoCliente() > 0)
            {
                ...
                try
                {
                    double temp = Convert.ToDouble(label12.Text) - totalAbonadoCliente();
                    // MessageBox.Show(""+temp);
                    string sql = string.Format("UPDATE CLIENTE SET CREDITO_DISPONIBLE = CREDITO_DISPONIBLE + {0}  WHERE ID_CLIENTE = (SELECT ID_CLIENTE FROM FACTURA WHERE ID_FACTURA = '{1}');", temp, textBox5.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    if (cmd.ExecuteNonQuery() == 1)
                    {
            ...
            else
            {
                ...
                try
                {
                    string sql = string.Format("UPDATE CLIENTE SET CREDITO_DISPONIBLE = CREDITO_DISPONIBLE + {0}  WHERE  ID_CLIENTE = (SELECT ID_CLIENTE FROM FACTURA WHERE ID_FACTURA = '{1}');", label12.Text, textBox5.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    if (cmd.ExecuteNonQuery() == 1)
                    {
                        //MessageBox.Show("SE ACTUALIZO CORRECTAMENTE EL CREDITO");
        ...
        private bool anulaCuenta()
        {
            ...
            try
            {
                string sql = string.Format("UPDATE CUENTA_POR_COBRAR_CLIENTE SET ESTADO_CUENTA = FALSE WHERE ID_FACTURA = '{0}';", textBox5.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
        ...
        private bool cajaFactura()
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_FACTURA FROM DET_CAJA WHERE ID_FACTURA = '{0}' AND ID_CAJA = {1};", textBox5.Text, codigoCaja);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private bool obtienSumCaja(double numero, string numeroCaja)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("UPDATE CAJA SET TOTAL_FACTURADO_CAJA = TOTAL_FACTURADO_CAJA - {0} WHERE ID_CAJA = {1};", numero, numeroCaja);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
        ...
        private bool obtienResCaja(double numero, string numeroCaja)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
        ...
        private int getNumeroCaja()
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_CAJA FROM DET_CAJA WHERE ID_FACTURA = '{0}' AND ESTADO_TUPLA = TRUE;", textBox5.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void guardaCaja(string cajaSeleccionada)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("UPDATE DET_CAJA SET ESTADO_TUPLA = FALSE WHERE ID_FACTURA = '{0}' AND ID_CAJA = {1};", textBox5.Text, cajaSeleccionada);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
        ...
        private void ajustaCaja()
        {
            if (cajaFactura())
            {
                //MessageBox.Show("la factura peertenece a la caja actual");
                if (Convert.ToDouble(label12.Text) > Convert.ToDouble(label12.Text))
                {

                    if (!obtienSumCaja(Convert.ToDouble(label12.Text) - Convert.ToDouble(label12.Text), codigoCaja))
                    {
                        MessageBox.Show("Los cambios se han realizado con exito!", "Devolución Envíos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        guardaCaja(codigoCaja);
                    }
                }
                else
                {
                    if (textBox6.Text == "CREDITO")
                    {
                    ...
                    {
                        if (obtienSumCaja(Convert.ToDouble(label12.Text), codigoCaja))
                        {
                            MessageBox.Show("Los cambios se han realizado con exito!", "Devolución Envíos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            guardaCaja(codigoCaja);
                        }
            ...
            {
                int temp = getNumeroCaja();
                if(temp > 0)
                {
                    MessageBox.Show("La factura seleccionada pertenece a otra caja los cambios se aplicaran !", "Devolución Envíos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (textBox6.Text == "CREDITO")
                    {
                       
                        MessageBox.Show("Los cambios se han realizado con exito!", "Devolución Envíos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        guardaCaja(Convert.ToString(temp));
                        

                    }
                    else
                    {
                        if (obtienSumCaja(Convert.ToDouble(label12.Text), Convert.ToString(temp)))
                        {
                            MessageBox.Show("Los cambios se han realizado con exito!", "Devolución Envíos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            guardaCaja(Convert.ToString(temp));
                            obtienSumCaja(Convert.ToDouble(label12.Text), codigoCaja);
                        }
                    }
                }
        ...
        private void revisaFactura()
        {
            ...
            try
            {
                string sql = string.Format("SELECT TIPO_FACTURA FROM FACTURA WHERE ID_FACTURA = '{0}';", textBox5.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    string temp = reader.GetString(0);
                    if (temp == textBox6.Text.Trim())
                    {
                         if (textBox6.Text == "CREDITO")
                         {
                            ...
                            {
                                DialogResult result;
                                result = MessageBox.Show("La factura actualmente posee una cuenta por cobrar, la cuenta por cobrar se eliminara!", "Devoluciones", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                if (result == System.Windows.Forms.DialogResult.OK)
                                {
                                    if (anulaCuenta())
                                    {
                                        // CREAR ALGORITMO PARA CALCULAR REPISIOCN DE CREDITOS
                                        MessageBox.Show("Los datos de la cuenta por cobrar y los creditos han sido actualizados!", "Devolciones", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        ajustaCaja();
                        ...
                        {
                            //MessageBox.Show("factura en efectivo");
                            ajustaCaja();
                        }
                    }
                    else
