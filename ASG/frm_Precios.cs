...
namespace ASG
{
    public partial class frm_Precios : Form
    {
        ...
        private void obtieneProducto(string codigo, string sucursal)
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT M.ID_MERCADERIA, M.ID_SUCURSAL, M.TOTAL_UNIDADES, M.DESCRIPCION,M.FECHA_INGRESO, M.ID_CLASIFICACION FROM MERCADERIA M WHERE ID_MERCADERIA = '{0}' AND M.ID_SUCURSAL = (SELECT ID_SUCURSAL FROM SUCURSAL WHERE NOMBRE_SUCURSAL ='{1}' AND ESTADO_TUPLA = TRUE LIMIT 1);", codigo, sucursal);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    label10.Text = reader.GetString(0);
                    label5.Text = "" + reader.GetDouble(2);
