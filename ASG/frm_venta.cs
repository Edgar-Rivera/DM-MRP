...
namespace ASG
{
    public partial class frm_venta : Form
    {
        ...
        private void frm_venta_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (Convert.ToInt32(e.KeyData) == Convert.ToInt32(Keys.Control) + Convert.ToInt32(Keys.O))
            {
                if (textBox2.Text != "")
                {
                        ...
                        try
                        {
                            ...
                            if (reader.Read())
                            {
                                ...
                                if (typeOF == "1001")
                                {
                                    ...
                                    var fomra = new frm_formaDeVenta("Venta al Crédito", "La venta al Cliente se efectuara según su credito disponible");
                                    fomra.Show();
                                }
                                else
                                {
                    ...
                    {
                        comboBox5.SelectedIndex = 0;
                        var fomra = new frm_formaDeVenta("Venta en Efectivo", "La venta al Cliente se efectuara en efectivo, el cliente debe cancelar el total de la venta");
                        fomra.Show();
                    }
                } else
                {
                    MessageBox.Show("NO HA SELECCIONADO NINGUN CLIENTE!", "NUEVA VENTA", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
        ...
        {
            var forma = new frm_clientes(nameUs, rolUs, usuario, idSucursal, true, textBox2.Text, textBox1.Text);
            forma.ShowDialog();
            if (forma.DialogResult == DialogResult.OK)
            {
            ...
            if (textBox2.Text != "")
            {
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        //dias = Convert.ToDouble(textBox10.Text); ;
                        string typeOF = reader.GetString(7);
                        codigoClientePersonal = reader.GetString(8);
                        iniciaVenta();
                        //textBox5.Focus();
                        textBox11.Focus();
                    }
        ...
        {
            // CARGA LSO DATOS DE UAN FACTURA EXISTENTE
        ...
        private bool obtieneConfig()
        {
            ...
            try
            {
                string sql = string.Format("SELECT IMPRESION_DEFECTO FROM CONFIGURACION_SISTEMA WHERE id_usuario = '{0}';", usuario);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void contruyeImpresion()
        {
            ...
            textc.Text = codigoClientePersonal;
        ...
        private void imprimePropiedades(CrystalReport4 rpt)
        {
            ...
            try
            {
                string sql = string.Format("SELECT IMPRESORA, COPIAS FROM CONFIGURACION_SISTEMA WHERE id_usuario = '{0}';",usuario);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private bool poseeVista()
        {
            ...
            try
            {
                string sql = string.Format("SELECT VISTA FROM CONFIGURACION_SISTEMA WHERE id_usuario = '{0}';",usuario);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                ...
                else if(comboBox5.SelectedIndex == 1)
                {
                    if (total > Convert.ToDouble(textBox9.Text))
                    {
                        ...
                        if (result == System.Windows.Forms.DialogResult.Yes)
                        {
                            guardaFactura();
                            actualizaCreditos();                           
                            var forma = new frm_ventaCredito(subtotal, descuento, total, textBox2.Text.Trim(), textBox1.Text.Trim(), textBox6.Text, codigoCaja, label9.Text, idSucursal);
                            if (forma.ShowDialog() == DialogResult.OK)
                            {
                                ...
                                if (existente)
                                {
                                    if (Convert.ToDouble(monto_factura) == Convert.ToDouble(label13.Text))
                                    {
                                        // LA FACTURA NO SUFRIO CAMBISO LOS CREDITOS SIGUEN IGUALES
                                    }
                                    else if (Convert.ToDouble(monto_factura) > Convert.ToDouble(label13.Text))
                                    {
                                        // EL TOTAL DE LA FACTURA HA SIDO MENOR AL ORIGINAL
                                        actualizaCreditoExistnete(Convert.ToDouble(monto_factura) - Convert.ToDouble(label13.Text));
                                    }
                                    else if (Convert.ToDouble(monto_factura) < Convert.ToDouble(label13.Text))
                                    {
                                        // EL TOTAL DEL AFACTUA ES MAYOR AL ORIGINAL
                                        actualizaCreditoRebaja(Convert.ToDouble(label13.Text) - Convert.ToDouble(monto_factura));
                                    }
                                }
                                else
                                {
                    ...
                    {
                        // EL CREDITO DEL CLINETE NO ES MAYOR A AL VENTA
                        // actualmente trabajdno en este
                        guardaFactura();
                        var forma = new frm_ventaCredito(subtotal, descuento, total, textBox2.Text.Trim(), textBox1.Text.Trim(), textBox6.Text, codigoCaja, label9.Text, idSucursal);
                        if (forma.ShowDialog() == DialogResult.OK)
                        {                          
        ...
        private void inventarioDex(double disponible, double cantidad, string codigo, string sucursal)
        {
            double restante = disponible - cantidad;
            if (actualizaDisponible(restante, codigo, sucursal))
            {
        ...
        private void algoritmoRecalculoMenor(string codigo, string sucursal, double cantidad)
        {
            ...
            try
            {
                string sql = string.Format("SELECT  TOTAL_UNIDADES, ID_SUCURSAL FROM MERCADERIA WHERE ID_MERCADERIA = '{0}' AND ID_SUCURSAL = (SELECT ID_SUCURSAL FROM SUCURSAL WHERE NOMBRE_SUCURSAL = '{1}' LIMIT 1);", codigo, sucursal);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    //MessageBox.Show(reader.GetString(0) + "  " +reader.GetDouble(1) + " " +reader.GetString(1));
                    double disponible = reader.GetDouble(0);
                    string sucursalD = reader.GetString(1);
                    //MessageBox.Show(reader.GetString(0) + "  " +reader.GetDouble(1) + " " +reader.GetString(1));
                    double restante = disponible + cantidad;

                    if (actualizaDisponible(restante, codigo, sucursalD))
                    {
        ...
        // AJUSTA LOS CREDITOS CUANDO SE HA SUPERADO EL LIMITE
        private void ajustaCreditos(double nuevoLimite, double diferencia)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
                string sql = string.Format("UPDATE CLIENTE SET CREDITO_DISPONIBLE  =  CREDITO_DISPONIBLE + {0}, LIMITE_CREDITO = {1} WHERE ID_CLIENTE  = '{2}';", diferencia, nuevoLimite,textBox2.Text.Trim());
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
        ...
        private void actualizaCreditoExistnete(double disponible)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
              
                string sql = string.Format("UPDATE CLIENTE SET CREDITO_DISPONIBLE  = CREDITO_DISPONIBLE + {0} WHERE ID_CLIENTE  = '{1}';", disponible, textBox2.Text.Trim());
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if ((cmd.ExecuteNonQuery() == 1) || (cmd.ExecuteNonQuery() == 0))
                {
                    //MessageBox.Show("SE resto corrctamente el credito");
        ...
        private void actualizaCreditoRebaja(double disponible)
        {
            OdbcConnection conexion = ASG_DB.connectionResult();
            try
            {
