...
namespace ASG
{
    public partial class frm_trasladoLotes : Form
    {
        ...
        private void cargaDatosProucto(string codigo)
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT M.DESCRIPCION, M.PRECIO_COMPRA, M.TOTAL_UNIDADES, (SELECT NOMBRE_PROVEEDOR FROM PROVEEDOR WHERE ID_PROVEEDOR = M.ID_PROVEEDOR) FROM MERCADERIA M WHERE ID_MERCADERIA = '{0}' AND ID_SUCURSAL = (SELECT ID_SUCURSAL FROM SUCURSAL WHERE NOMBRE_SUCURSAL = '{1}');",codigo, comboBox1.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    dataGridView4.Rows.Add(codigo, comboBox1.Text, reader.GetString(0), reader.GetString(1), reader.GetString(2),0, reader.GetString(3));
                }
            }
            catch (Exception e)
            {
        ...
        private void frm_trasladoLotes_KeyDown(object sender, KeyEventArgs e)
       {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (comboBox1.Text != "")
            {
                var forma = new frm_seleccionaProductosdTraslado(3,comboBox1.Text,"ADMINISTRADOR");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if(textBox1.Text != "" & Convert.ToDouble(textBox1.Text)> 0)
            {
                ...
                {
                    MessageBox.Show("NO HA INGRESADO NINGUN PRODUCTO PARA TRASLADAR!", "TRASLADOS MERCADERIA", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            } else
            {
                MessageBox.Show("INGRESE UNA CANTIDAD VALIDA!", "TRASLADOS MERCADERIA", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult result;
                result = MessageBox.Show("¿ESTA SEGURO QUE DESEA CANCELAR LA OPERACION?, LOS VALORES SE RESTAURARAN CON 0!", "TRASLADO DE MERCADERIA", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == System.Windows.Forms.DialogResult.Yes)
                {
                    textBox1.Text = "";
                    for (int i = 0; i < dataGridView4.RowCount; i++)
