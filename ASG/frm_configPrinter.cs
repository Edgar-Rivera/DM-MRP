...
namespace ASG
{
    public partial class frm_configPrinter : Form
    {
        ...
        bool Dragging;
        string usuario;
        public frm_configPrinter(string user)
        {
            InitializeComponent();
        ...
        private void cargaAjustes()
        {
            ...
            try
            {
                ...
                string sql = string.Format("SELECT IMPRESORA, COPIAS, VISTA FROM CONFIGURACION_SISTEMA WHERE id_usuario = '{0}';", usuario);
            ...
            if(comboBox5.Text != "")
            {
                if(Convert.ToDouble(textBox1.Text.ToString()) > 0)
                {
                    ...
                    try
                    {
                        ...
                        string sql = string.Format("UPDATE CONFIGURACION_SISTEMA SET IMPRESORA = '{0}' , COPIAS = {1} , VISTA = {2} WHERE id_usuario = '{3}' ;", comboBox5.Text.Trim(), textBox1.Text, checkBox1.Checked, usuario);
                        OdbcCommand cmd = new OdbcCommand(sql, conexion);
                ...
                {
                    MessageBox.Show("EL NUMERO DE COPIAS DEBE SER MAYOR A CERO", "CONFIGURACION IMPRESION", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            } else
            {
