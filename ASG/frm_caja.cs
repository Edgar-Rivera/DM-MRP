...
namespace ASG
{
    public partial class frm_caja : Form
    {
        ...
        private void my_menu_Itemclicked(object sender, ToolStripItemClickedEventArgs e)
        {
            if (flag == false)
            {
                ...
                else if (e.ClickedItem.Name == "ColDet")
                {
                    ...
                    {
                        bool cast = false;
                        if (dataGridView1.CurrentRow.Cells[6].Value.ToString() == "CERRADA")
                        {
                            cast = true;
                        }
        ...
        private void cargaActuales()
        {
            ...
            {
                dataGridView1.Rows.Clear();
                string sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE APERTURA_CAJA  >= curdate() AND NOMBRE_SUCURSAL = '{0}' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void buscaActuales()
        {
            if (textBox1.Text != "")
            {
                ...
                {
                    dataGridView1.Rows.Clear();
                    string sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE  NOMBRE_SUCURSAL = '{0}' AND NOMBRE_USUARIO LIKE '%{1}%' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text, textBox1.Text.Trim());
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
                    ...
                    } else
                    {
                        ...
                        sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE  NOMBRE_SUCURSAL = '{0}' AND APERTURA_CAJA LIKE '%{1}%' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text, textBox1.Text.Trim());
                        ...
                        } else
                        {
                            ...
                            conexion.Close();
                            conexion = ASG_DB.connectionResult();
                            sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE  NOMBRE_SUCURSAL = '{0}' AND MONTO_APERTURA_CAJA LIKE '%{1}%' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text, textBox1.Text.Trim());
                            cmd = new OdbcCommand(sql, conexion);
                            reader = cmd.ExecuteReader();
                            if (reader.Read())
                            {
                            ...
                            } else
                            {
                                ...
                                conexion.Close();
                                conexion = ASG_DB.connectionResult();
                                sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE  NOMBRE_SUCURSAL = '{0}' AND TOTAL_FACTURADO_CAJA LIKE '%{1}%' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text, textBox1.Text.Trim());
                                cmd = new OdbcCommand(sql, conexion);
                                reader = cmd.ExecuteReader();
                                if (reader.Read())
                                {
                                ...
                                } else
                                {
                                    ...
                                    conexion.Close();
                                    conexion = ASG_DB.connectionResult();
                                    sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE  NOMBRE_SUCURSAL = '{0}' AND OBSERVACIONES_CAJA LIKE '%{1}%' ORDER BY APERTURA_CAJA DESC;", comboBox1.Text, textBox1.Text.Trim());
                                    cmd = new OdbcCommand(sql, conexion);
                                    reader = cmd.ExecuteReader();
                                    if (reader.Read())
                                    {
        ...
        private void cargaActualesFecha()
        {
            ...
            {
                dataGridView1.Rows.Clear();
                string sql = string.Format("SELECT * FROM VISTA_CAJAS WHERE APERTURA_CAJA LIKE '%{0}%' AND NOMBRE_SUCURSAL = '{1}' ORDER BY APERTURA_CAJA DESC;", dateTimePicker1.Value.ToString("yyyy-MM-dd"),comboBox1.Text);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
