...
namespace ASG
{
    public partial class frm_abono : Form
    {
        ...
        string codigoCaja;
        public frm_abono(string cuenta, double total, string codigo, bool estado, string sucursal, string usuario, string caja)
        {
            InitializeComponent();
            numeroCuenta = cuenta;
            totalFactura = total;
        ...
        private void abonaPagoCliente()
        {
            ...
            try
            {
                string sql = string.Format("INSERT INTO PAGO_CLIENTE VALUES (NULL,{0},{1},{2},{3},NOW(),TRUE,TRUE,{4});", numeroCuenta, 0, textBox5.Text, textBox5.Text, nombreSucursal);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
