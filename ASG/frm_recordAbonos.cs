...
namespace ASG
{
    public partial class frm_recordAbonos : Form
    {
            ...
            if (dataGridView1.RowCount > 0)
            {
                if (estadoC)
                {
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        adressUser.ingresaBitacora(nombreSucursal, nombreUsuario, "ELIMINA PAGO CUENTA", codigoProveedor);
                        cargaDatos();
                        abonado = 0;
                        getAbonos();
                        setterForm();
                ...
                {
                    DialogResult result;
                    result = MessageBox.Show("EL TOTAL DE LA COMPRA HA SIDO PAGADO! , SI ELIMINA EL ABONO AL PROVEEDOR LA CUENTA SE ACTIVARA Y TENDRA UN SALDO PENDIENTE, ¿ESTA SEGURO QUE DESEA ELIMINAR EL ABONO AL PROVEEDOR?", "CUENTAS POR PAGAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
        ...
        private void activaCuenta()
        {
            ...
            try
            {
                string sql = string.Format("CALL ACTIVA_CUENTA('{0}');", numeroCuenta);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    var forma = new frm_creditoActualizado();
                ...
                {
                    MessageBox.Show("IMPOSIBLE ACTIVAR DE NUEVO LA CUENTA", "CUENTAS POR PAGAR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void frm_recordAbonos_Load(object sender, EventArgs e)
        {
