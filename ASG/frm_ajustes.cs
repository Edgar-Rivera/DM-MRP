    ...
    public partial class frm_ajustes : Form
    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (textBox11.Text != "" & textBox10.Text != ""  & textBox12.Text != "" & textBox9.Text != "" & comboBox1.Text != "" )
            {
                ...
                {
                    // el nombre cambio
                   // MessageBox.Show("se ira por uqe cabio el combo");
        ...
        private void cargaConfiguracion()
        {
            ...
            try
            {
                string sql = string.Format("SELECT * FROM  CONFIGURACION_SISTEMA WHERE id_usuario = '{0}';", usuario);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void button9_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = sql = string.Format("CALL UPDATE_CONFIG('{0}',{1},{2},{3},{4},{5},{6},{7});",usuario,checkBox1.Checked, checkBox2.Checked,checkBox4.Checked,checkBox6.Checked,checkBox13.Checked,checkBox14.Checked, checkBox15.Checked);
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    timerActions();
        ...
        {
            var forma = new frm_configPrinter(usuario);
            forma.ShowDialog();
        }
        ...
        private void dataGridView2_KeyDown(object sender, KeyEventArgs e)
        {
            if(e.KeyData == Keys.Enter)
            {
                ...
                if (dataGridView2.RowCount > 0)
                {
                    var forma = new frm_vistaSucursal(dataGridView2.CurrentRow.Cells[0].Value.ToString());
                    forma.Show();
                }
            }
        }
